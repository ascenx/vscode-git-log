...
- Current-line history shows three unchanged lines above and below each change by default while keeping the Commit list limited to changes that affected the selected line. Configure `gitLogWorkbench.lineHistory.contextLines` from `0` to `20` to adjust the displayed context.
